<?php
  ...
  function fill_election_cycles ($cycles) {
    ...
    while ($row = $result->fetch_assoc()) {
      ...
      echo "<div class=\"caps_cycle_box\">";
      if (in_array ($cycle_start, $cycles)) {echo "<input type=\"checkbox\" id=\"y{$cycle_start}\" name=\"cycles[]\" value=\"{$cycle_start}\" onFocus=\"document.getElementById('cycle_dates').checked=true;\" CHECKED class=\"left caps_radio6\" alt=\"Election Cycle {$cycle_start}\">";} else {echo "<input type=\"checkbox\" id=\"y{$cycle_start}\" name=\"cycles[]\" value=\"{$cycle_start}\" class=\"caps_radio6\" onFocus=\"document.getElementById('cycle_dates').checked=true;\" class=\"left caps_radio6\" alt=\"Election Cycle {$cycle_start}\">";}
      echo "<label for=\"y{$cycle_start}\" class=\"font_input caps_label6\">$cycle_start-$cycle_end</label>";
      echo "</div>";
    }
  }
?>