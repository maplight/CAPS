<?php
  ...
  function fill_propositions ($selected) {
    ...
    $last_election = "";
    foreach ($web_db->query ("SELECT DISTINCT Election, Target FROM smry_propositions WHERE Election > '2001-01-01' ORDER BY Election DESC, Target") as $row) {
      ...
      if ("{$row["Election"]}#" . str_replace ("\"", "&quot;", $row["Target"]) == $selected) {echo "<OPTION VALUE=\"{$row["Election"]}#" . htmlentities ($row["Target"]) . "\" SELECTED>&nbsp;&nbsp;&nbsp;&nbsp;{$row["Target"]}</OPTION>";} else {echo "<OPTION VALUE=\"{$row["Election"]}#" . htmlentities ($row["Target"]) . "\">&nbsp;&nbsp;&nbsp;&nbsp;{$row["Target"]}</OPTION>";}
  ...
  function fill_qs_elections () {
    global $web_db;
    foreach ($web_db->query ("SELECT DISTINCT Election FROM smry_propositions WHERE Election > '2001-01-01' ORDER BY Election DESC, Target") as $row) {
