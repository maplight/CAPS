...
truncate table ftp_f501_502_cleaned;
insert ftp_f501_502_cleaned (
    filing_id
  , amend_id
  , form_type
  , filer_id
...
delete from ftp_f501_502_cleaned
where
  form_type <> 'F501'
  or session < 2000 
...
from 
  ...
  join (
    ...
    from 
      ftp_f501_502_cleaned a
      join (
        select 
            filer_id
          , session
          , max(rpt_date) as rpt_date
        from ftp_f501_502_cleaned
        group by 
            filer_id
          , session
