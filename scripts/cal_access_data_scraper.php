<?php
  ...
  $ps_check = trim (shell_exec ("ps aux | grep 'php cal_access_data_scraper.php' | grep -v grep | wc -l"));
  ...
  if ($ps_check == "1") {
    ...
    $query = "SELECT * FROM cal_access_sessions ORDER BY session DESC";
    ...
    while ($row = $result->fetch_assoc()) {
      ...
      if ($process_session) {
        ...
        script_query ("UPDATE cal_access_sessions SET last_ran = NOW() WHERE session = '$session';"); 
      }
    }
